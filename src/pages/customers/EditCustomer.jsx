...
import { User, Building, Mail, Phone, MapPin, AlignLeft, ArrowLeft, Loader2 } from 'lucide-react';
...
const EditCustomer = () => {
  ...
  // Hydrate form values on fetch completion
  useEffect(() => {
  ...
  if (error) {
    return (
      <div className="text-center text-slate-500 py-12">
        <p className="font-semibold text-slate-350">Failed to load customer profile</p>
        <p className="text-sm mt-1">{error.message}</p>
        <Link to="/customers" className="text-brand-light text-sm mt-4 hover:underline block">
          Back to list
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="flex items-center space-x-3 mb-2">
        ...
        <div>
          <h2 className="text-3xl font-extrabold text-white tracking-tight">Edit Customer</h2>
          <p className="text-slate-400 text-sm mt-1">Modify client profile details.</p>
        </div>
      </div>
